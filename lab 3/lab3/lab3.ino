...
void rightWall() {
  ...
  double Le = readIRLeft();     //read left IR
  double Ri = readIRRight();    //read right IR
  boolean F = Fr < 5;           //logical for front IR sensing object
  boolean L = Le < thresh;      //logical for left IR sensing object
  ...
  if (count > 8) {
    ...
    if (Ri > 6) {
      spin('R', bangAngle, dSpd);
      drive(straight, dSpd);
      count++;
      rightWall();

    } else if (Ri < 4) {
      spin('L', bangAngle, dSpd);
      drive(straight, dSpd);
      count = 0;
      rightWall();
...
void leftWall() {
  ...
  double Le = readIRLeft();     //read left IR
  double Ri = readIRRight();    //read right IR
  boolean F = Fr < 4;          //logical for front IR sensing object
  boolean R = Ri < thresh;      //logical for right IR sensing object
...
void randomWander() {
  ...
  double vecX = random(64);                         //randomize a x coordinate with 50% chance of being left or right
  double vecY = random(128);                        //randomize a y coordinate with 75% chance of being forward
